...
Поля и фильтры `taxi`:
- `direction` — тип оффера: `1=город`, `2=межгород`, `3=груз`.
- `routeDirection` — направление межгорода: `1=Цхинвал -> Владикавказ`, `2=Владикавказ -> Цхинвал`.
- `fromPlace`, `toPlace`, `routeText` — явные поля маршрута для карточек и безошибочного определения направления.
- `departureAt` — дата/время выезда.
- `departureFrom` в `getTaxiOffers` — вернуть только поездки не раньше выбранного времени выезда.
